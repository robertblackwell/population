...
// 	growthRates := CalculateGrowthRates(geoPopulations, currentBaseYearPopulation)
...
// func processPopulationResponse(allPopulations []LadPopulationProjection, growthRates map[string]map[string][]float64, startYear, requestedOffset int, includeIntermediates bool) (*models.PopulationProjections, error) {

// 	// Calculate the projected population for each LAD and age range
// 	projectedPopulation := []Result{}
// 	for _, pop := range allPopulations {
// 		code := pop.Code
...
// 				futurePopulation := math.Round(float64(currentPopulation) * (1 + (growthRate / 100)))
// 				projected[i] = int(futurePopulation)
// 			}
